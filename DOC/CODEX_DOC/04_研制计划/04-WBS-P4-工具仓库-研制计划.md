...
- 已建立独立方案线
- 已存在节点设计与 issue 镜像
- `P4` 已按 `P2` 的组织形态整改为“稳定主软件设计说明 + 时间版补充设计”结构
- 模拟研制执行器、工具仓测试治理、`P3-sim` 典型工单发生器、自演进巡检闭环、真实工具落地验证等专题已归入主设计和补充设计体系
- 已建立 `P4` 设计到模块 / 类 / API / 测试的正式映射表
- `P4.2` 输入工序链闭环、`P4.3` 自演进巡检闭环、`P4.4` 后端服务化与运行时演进已完成当前规划范围内的代码落地
- `P4.5` 已进入真实最小工具交付闭环设计与首版实现映射范围
- 当前文档状态：主设计已按目标组织形态整改完成，后续编码与验收以 `P4-工具仓库设计.md` 为主入口
