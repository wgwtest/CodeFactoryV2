...
模块章节数量和名称必须从需规事实与 `design_package.module_designs` 动态生成，以上第 4 至第 8 章只是空域协同规划样例。不得固定使用 SX-DataStore 或其他样例模块名。

不得再把“前端软件设计”“后端软件设计”作为一级主章节。前端交互、后端服务、数据对象、接口、状态和质量约束必须放入对应业务功能模块章节的二级内容。确需说明横切技术机制时，可在“横切技术机制”章节中说明通用前端框架、通用后端服务分层、统一权限、审计、异常处理和集成边界。
...
- `result_json.design_document.sections` 至少包含目标软设主要章节，且主体章节必须按业务功能模块展开。
...
- `design_document.sections` 的模块章节应与 `design_package.module_designs[].name` 对齐；不得继续以“总体架构/前端软件设计/后端软件设计/API 设计”等横切章节作为正文主体。
...
- 软设正文不得以“前端软件设计/后端软件设计”横切章节替代功能模块展开；横切章节只能说明通用技术机制，且不得作为一级主章节。
- 输出不得混入与输入需规无关的样例领域事实；例如空域协同规划输入不得出现 SX-DataStore 的消费者、生产者、管理者、资源申请、审批交付、数据资源目录等样例内容。
