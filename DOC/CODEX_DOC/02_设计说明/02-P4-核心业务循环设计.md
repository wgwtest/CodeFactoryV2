...
虽然本文件主焦点是 `P4` 的核心业务循环，但 `P4` 当前已被接受的工作台视觉语言，已经成为 `P5` 继续继承的阶段基线，因此不能只留在工作层过程稿或前端样式文件里，必须在正式文档层固定。

当前正式口径以**已被接受的前端实现与样式测试**为准，而不再以早期过程稿中的“深色英雄区”描述为准。
...
- 阴影与描边保持克制，强调结构秩序而不是装饰感
- 响应式下由多列收敛为单列，但不改变信息层级
...
- `apps/web/src/components/p4/p4-page.css`
- `apps/web/src/test/P4WorkspaceStyle.test.ts`

继承规则固定为：

- `P5` 阶段工作台默认继承这组工作台视觉语言
- 节点级设计只记录“继承 + 节点例外”，不重复定义整套阶段风格
