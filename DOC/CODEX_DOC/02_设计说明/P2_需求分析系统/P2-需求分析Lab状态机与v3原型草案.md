# P2 Requirement Analysis Orchestrator Lab 状态机与 v3 原型草案

**日期：** 2026-05-01

**文档状态：** 草案，待用户评审

**目标：** 修正当前 `P2 Requirement Analysis Orchestrator Lab` 的状态机和页面结构，使“组织器配置、会话管理、当前 Turn、调用日志”成为显式视图切换，而不是由业务数据变化自动切换界面状态。
...
业务数据状态机描述 `Requirement Analysis Orchestrator Lab` 的后台业务进程：组织器加载、会话创建、Turn 运行。
...
```text
┌────────────────────────────────────────────────────────────────────┐
│ P2 Requirement Analysis Orchestrator Lab                                                │
