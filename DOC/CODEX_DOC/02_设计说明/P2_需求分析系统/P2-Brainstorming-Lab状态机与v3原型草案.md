# P2 Brainstorming Lab 状态机与 v3 原型草案

**日期：** 2026-05-01

**文档状态：** 草案，待用户评审

**目标：** 修正当前 `P2 Brainstorming Lab` 的状态机和页面结构，使“组织器配置、会话管理、当前 Turn、调用日志”成为显式视图切换，而不是由业务数据变化自动切换界面状态。
...
2. **业务状态错误驱动了界面 Tab。**
   - 点击“启动验证”后，系统创建了 `BrainstormSession`，左侧对象树就自动高亮“会话对象”。
   - 发送输入产生 `BrainstormTurn` 后，左侧对象树又自动高亮或强化 “Turn 对象”。
...
业务数据状态机描述 `Brainstorming Lab` 的后台业务进程：组织器加载、会话创建、Turn 运行。
...
> 2026-05-01 修订：当前 Turn 的内容基线以 `P2-Brainstorming-Turn引擎与状态机设计.md` 为准。Turn 从用户输入开始，上一轮建议话题只是上下文；本视图不再表达为“上一轮问题 / 用户回答 / 当前节点关闭”的问卷式模型。

```text
┌────────────────────────────────────────────────────────────────────┐
│ 当前 Turn 决策审计                                                 │
│ BrainstormTurn: turn-0001                                          │
├────────────────────────────────────┬───────────────────────────────┤
│ 本轮沟通对象                       │ Brainstorming Service 循环     │
│                                    │                               │
│ previous_suggestion                │ 1. 接收用户输入                │
│ 上轮建议：确认系统目标用户         │ 2. 读取会话状态                │
│                                    │ 3. 读取上一轮用户关注点        │
│ previous_user_focus                │ 4. 读取上一轮建议话题          │
│ 上轮用户关注：先解释组织器逻辑     │ 5. 判断承接关系                │
│                                    │ 6. 解释用户意图                │
│ user_input                         │ 7. 调用 Provider               │
│ 先不谈目标用户，先修正系统定位     │ 8. 解析结构化输出              │
│                                    │ 9. 校验并落状态                │
│ input_relation                     │ 10. 提取本轮用户关注点         │
│ topic_shift                        │ 11. 生成下一轮建议话题         │
│                                    │                               │
│ organizer_interpretation           │ affected_spec_nodes            │
│ 用户要求优先澄清系统定位           │ REQ-2.1 软件定位               │
│                                    │                               │
│ closure_assessment                 │ state_delta / patch / risks    │
│ 已闭环用户本轮纠正诉求             │                               │
│                                    │                               │
│ next_suggestion                    │ decision_trace                 │
│ 可为空或建议下一轮方向             │ 本轮判断依据                   │
└────────────────────────────────────┴───────────────────────────────┘
```
...
- `承接话题`：上一轮建议话题；首轮显示“无，用户自由发起”。
- `上一轮用户关注点`：上一轮用户真正提出的问题、要求或纠正点；首轮显示“无”。
- `本轮用户输入`：原文。
- `输入关系判断`：采纳、部分承接、改题、反驳、补充、无关等。
- `系统理解与回应`：组织器如何理解用户输入，以及给用户的回应。
- `本轮闭环判断`：是否回应了用户本轮关心的问题。
- `影响的规格节点`：本轮投影到哪些需求规格章节，可为 0、1 或多个。
- `本轮状态变化`：事实、问题、Patch、风险、注记。
- `下一轮建议话题`：可为空，不是强制问题。
- `决策依据`：组织器的判断链。
...
│ status: mocked                     │ Model: mock-brainstorm-v1     │
│ model: mock-brainstorm-v1          │ Status: mocked                │
│ time: 2026-05-01 ...               │                               │
│                                    │ raw_model_response             │
│ call-0002                          │ {                             │
...
| 点击启动验证 | 创建 `BrainstormSession` | 不变化 | 当前 Tab 显示“会话已创建”；会话管理 Tab 出现徽标 |
| 点击会话管理 Tab | 无业务变化 | 切到会话管理 | 显示 CLI 问答区 |
| 发送输入 | 创建 `BrainstormTurn`，更新会话状态 | 不变化 | user 消息立即上屏；Provider 返回后会话消息流和摘要更新；当前 Turn Tab 出现最新 Turn 徽标 |
| 点击快捷选项的“选择 X”按钮 | 按普通用户输入创建 `BrainstormTurn` | 不变化 | 转化为 `X，选项文本` 并进入消息流；选项行本身点击不触发 |
...
9. Lab 仍保持独立路由 `/p2-brainstorm-lab`，不进入 MainShell。
