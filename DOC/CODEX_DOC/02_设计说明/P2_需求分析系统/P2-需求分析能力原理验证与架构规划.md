...
本阶段不把 `Requirement Analysis` 直接嵌入当前 `RequirementAuthoringPage`，而是先建立独立的 `P2 Requirement Analysis Orchestrator Lab`。
...
- 专家工作台是二者的前端组合视图。
- `P2 Requirement Analysis Orchestrator Lab` 只消费 `Requirement Analysis Service`，用于验证原理，不写入正式需求规格文档。
...
展示能力
  - 专家工作台
  - Requirement Analysis Orchestrator Lab
  - 配置台
```
...
## 5. P2 Requirement Analysis Orchestrator Lab

### 5.1 页面目的

`P2 Requirement Analysis Orchestrator Lab` 是独立的原理验证页面，用于测试 `Requirement Analysis` 能力本身。
...
`P2 Requirement Analysis Orchestrator Lab` 的 CLI 问答区应接近命令行式协作，而不是重型问卷。
...
- 先建立独立 `P2 Requirement Analysis Orchestrator Lab`，不直接改造当前专家工作台。
