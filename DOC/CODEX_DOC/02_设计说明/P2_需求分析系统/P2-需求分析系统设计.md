...
| `Requirement Analysis Orchestrator Lab` | 需求分析组织器 Lab | 需求分析人员 / 开发者 | 验证问答式需求收敛、Turn 协议和可替换组织器 |
...
### 10.5 强规则组织器与 Brainstorming 组织器
...
| `xg-brainstorming-orchestrator` | 对齐启发式协作方式，以更聪明的问题组织、选项建议和方向判断支持需求收敛 | 输出更有启发性、更少机械追问 |
...
| `BrainstormTurn` | 容易绑定 brainstorming 方法 | `RequirementAnalysisTurn` |
...
测试文件名可以随模块命名迁移逐步调整，但测试覆盖口径不应降低。
