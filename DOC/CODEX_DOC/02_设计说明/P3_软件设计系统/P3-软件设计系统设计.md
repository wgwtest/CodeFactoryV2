...
**最近更新：** 2026-05-16，归入软设工作区合并 Tab 与 Canvas 验证方案，明确 `SoftwareDesignPackage` 是软设事实源，A4 正文、功能树、分层架构、真实技术实现、展示形态和 `P4` 投影均为同一设计包的不同投影或内部形态；前端展示可以一体化，但模块职责、对象模型、命令边界和渲染平台必须解耦。
...
当前首版代码落地中，`P3DesignLabPage` 只调用 `buildP3DesignMorphModel(workbench)` 装配软设形态工作区，不再在页面内拼接 `需规 -> 软设文档 -> 功能树 -> 分层架构 -> 技术实现 -> 展示形态 -> P4 投影` 的业务链。`p3DesignMorphAdapter.ts` 是该形态链的唯一前端适配入口，负责把工作台 ViewModel 中的输入事实、软件设计说明、设计基线摘要和投影树摘要转换为形态节点与窗口。

`DesignMorphCanvasPlatform` 只消费形态节点、窗口、当前窗口 ID 和窗口变更事件。它不得读取 `P3DesignLabSession`、`SoftwareDesignPackage`、`P3DesignLabInputPackage` 等业务 DTO，也不得自行推导 P3 的七段业务形态。Canvas 内部渲染通过 `designMorphRenderers.ts` 的注册表按 `layoutKind` 分发；当前首版注册表使用 `paper / tree / architecture / table / cards` 等基础展示类型承载形态，后续可以在保持注册表接口不变的前提下拆分为 `DesignDocumentStageRenderer`、`FunctionalTreeStageRenderer` 等独立文件。
...
形态链默认采用“双元素滑窗”展示：一个窗口只展示两个相邻形态，例如 `需规 -> 软设文档`、`软设文档 -> 功能树`、`功能树 -> 分层架构`、`展示形态 -> P4 投影`。顶部滑窗轴用于定位当前窗口，主体视口用于查看和编辑当前窗口，右侧 Inspector 用于处理当前选中对象的追溯、检查和动作。
...
| 功能树形态渲染器 | `FunctionalTreeStageRenderer` | 渲染功能节点、父子关系、覆盖状态 | 软件设计基线模块 |
...
| 选中对象检查器 | `SelectedObjectInspector` | 展示当前对象追溯、检查项和可执行动作 | 读取当前形态 ViewModel，命令回到所属模块 |
...
首版实现允许把多个基础展示形态集中到 `designMorphRenderers.ts` 注册表中，但该注册表只能按展示类型渲染图元，不得承担业务推导。业务形态和展示形态之间的映射必须由 `p3DesignMorphAdapter.ts` 输出的 `layoutKind` 明确表达。后续拆分独立 Renderer 时，应从注册表中逐步抽出渲染函数，而不是让 `P3DesignLabPage` 或 `DesignMorphCanvasPlatform` 重新硬编码业务分支。

多分支协作时按以下边界分工：

| 分支 / 子系统方向 | 可写范围 | 不可写范围 | 合并接口 |
| --- | --- | --- | --- |
| Canvas 视口平台 | `DesignMorphCanvasPlatform`、视口状态、文本测量、命中检测、渲染调度 | 任何 `SoftwareDesignPackage` 业务字段、后端 API 合同 | `CanvasNodeViewModel`、`CanvasEdgeViewModel`、选中事件 |
...
任何分支需要跨边界读取数据时，必须先扩展 ViewModel 或模块命令合同，不得通过查询 DOM、Canvas 图元私有字段、React 组件内部 state 或临时全局变量绕过边界。
...
- 说明展示方案与功能节点、技术模块、原型图和后续 `P6` 展示投影之间的关系。
...
Canvas 长卷中的“需规、软设文档、功能树、分层架构、技术实现、展示形态、`P4` 投影”不是固定装饰块，而是可独立迭代的形态节点。每个形态节点应具备节点级布局状态，首版支持通过标题栏或右上角控制点拖拽，通过右下角把手缩放。节点布局状态只属于前端 `canvasViewportState`，不得改变软件设计说明正文、设计基线、功能树、技术实现或 `P4` 投影的业务事实。
...
| `SelectedObjectInspector` | 当前选中对象、追溯、检查和动作面板 |
...
设计投影与交换模块负责把软件设计包投影给下游系统。它不产生新的设计事实，只负责按下游合同生成不同视图。
...
- `activeDesignId`
- `activeMorphWindow`：当前双元素形态窗口，例如 `requirement_to_document`、`document_to_functional_tree`、`functional_tree_to_layered_architecture`、`presentation_shape_to_p4_projection`
...
当前前端实现中，软设形态链节点以 `DesignMorphStageViewModel` 输入 `DesignMorphCanvasPlatform`。该对象是 Canvas 平台可消费的共性展示模型，不是后端权威对象，至少包含：
...
| `GET` | `/software-designs/{software_design_id}/functional-tree` | 目标态：读取功能树视图数据 |
| `GET` | `/software-designs/{software_design_id}/layered-architecture` | 目标态：读取分层架构视图数据 |
| `GET` | `/software-designs/{software_design_id}/technical-implementation` | 目标态：读取真实技术实现视图数据 |
| `GET` | `/software-designs/{software_design_id}/presentation-shapes` | 目标态：读取展示形态视图数据 |
    ...
    -> 用户选择一条需规
      -> 前端读取需规详情和关联软设列表
        -> 用户打开既有软设或新建软设
          -> 前端加载软设工作区
            -> 展示统一软设工作区形态链
              -> 通过双元素滑窗查看需规、软设文档、功能树、分层架构、技术实现、展示形态和 P4 投影
```
