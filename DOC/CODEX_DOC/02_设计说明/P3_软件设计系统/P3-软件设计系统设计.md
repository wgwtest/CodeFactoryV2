...
>
> 本文不承担实现审查和迁移计划职责；本轮重构依据见 `P3-软件设计系统设计-260510-目标态文档重构补充.md`。
>
> `P3-Design-Lab-v2设计.md` 是 `P3 Design Lab` 专项设计文档，保留为新版设计链路的原理验证说明，不替代本文的系统级主设计。
...
`P3 Design Lab` 是这条主路径的首个原理验证入口。旧 `/xx-p3` 订单、审批、评审和推送能力保留为历史流程面和后续治理侧线，但不再作为新版 `P3` 的主架构起点。
...
| 5 | 设计基线视图 | `Design Baseline View` | 前端派生对象 | 展示架构、模块、接口、数据、状态、部署和追溯等结构化设计事实。 |
...
| 10 | 阶段文档工作台 | `StageDocumentWorkbench` | 前端共性容器组件 | 承载阶段标题、动作、左侧输入事实、左侧交互区和右侧标准文档主产物区。 |
| 11 | 阶段工作台视图模型 | `StageDocumentWorkbenchViewModel` | 前端 ViewModel | 把阶段身份、输入事实、交互、标准文档、质量门禁、投影和动作统一为共性组件输入。 |
    ...
    -> View Model Builder
      -> Workbench Adapter
        -> StageDocumentWorkbench
          -> Stage Input / Interaction / Document Product Panels
```
...
`P3` 前端目标态采用阶段文档工作台类群，而不是为 `P3 Design Lab` 单独维护一套页面结构。类群分为六类：

| 类群 | 代表命名 | 职责 |
| --- | --- | --- |
| ViewModel 类群 | `StageDocumentWorkbenchViewModel`、`StandardDocumentViewModel`、`StageQualityGateViewModel` | 定义共性工作台和标准文档主产物需要的数据结构。 |
| Adapter 类群 | `p3DesignLabWorkbenchAdapter`、后续 `p2RequirementAuthoringWorkbenchAdapter` | 把阶段业务 DTO 转换为共性 ViewModel，隔离业务字段和 UI 组件。 |
| Workbench 容器组件 | `StageDocumentWorkbench` | 承载标题、状态、动作、左侧输入事实、左侧交互和右侧主产物布局。 |
...
- 页面编排层只负责副作用和命令提交，不负责正文、检查、投影的细粒度渲染。
...
| 路由 | 页面 | 定位 |
| --- | --- | --- |
| `/p3-design-lab` | `P3 Design Lab` | 新版虚规到软设原理验证入口 |
...
1. 左上：需求规格说明输入事实源。
2. 左下：自然语言配置 / CLI。
3. 右侧：软件设计说明 A4 正文和设计基线 / 工单投影侧栏。

右侧软件设计说明是视觉主产物，应接近 A4 文档形态。CLI 是转换策略和局部修订通道，应比普通聊天输入更像设计命令台，显示状态链路和运行日志。

`P3 Design Lab` 不应拥有独立于共性工作台的页面骨架。它应作为 `StageDocumentWorkbench` 的 `P3` 适配实例：
...
该结构保证 `P3` 的首轮验证结果可以回迁 `P2`，也可以在正式 `/p3-design` 工作台中复用。
...
- 输入事实源区进入设计输入模块。
- CLI 和轮次日志进入设计会话与运算模块。
- A4 软设正文进入软件设计说明编写模块。
- 右侧基线摘要和工单投影进入基线模块与投影交换模块。
...
| `StageDocumentWorkbench` | 阶段工作台外壳和左右布局 |
...
### 7.10 前端阶段文档工作台 ViewModel 对象

本节对象不是持久化对象，也不是后端返回 DTO。它们是前端 Adapter 从后端权威状态派生出来的工作台显示合同，用于把 `P3` 页面接入平台级阶段文档工作台。

#### 7.10.1 `StageDocumentWorkbenchViewModel`

`StageDocumentWorkbenchViewModel` 是工作台根视图模型，至少包含：
...
`P3` 中，该对象由 `p3DesignLabWorkbenchAdapter` 根据 `P3DesignLabInputPackage` 和 `P3DesignLabSession` 生成。

#### 7.10.2 `StandardDocumentViewModel`
...
#### 7.10.3 `StageQualityGateViewModel`
...
#### 7.10.4 `StageOutputProjectionViewModel`
...
`P3` 中，该对象用于展示 `P4` 模块工单投影和后续 `P5` 设计输入投影。

#### 7.10.5 `StageInteractionViewModel`
...
该对象只抽象交互展示和影响范围，不抽象 `P2` 或 `P3` 的组织器算法。

#### 7.10.6 `StageFreezeViewModel`
...
| Adapter | 输入 | 输出 | 约束 |
| --- | --- | --- | --- |
| `p3DesignLabWorkbenchAdapter` | `P3DesignLabInputPackage`、`P3DesignLabSession` | `StageDocumentWorkbenchViewModel` | 不调用 API，不访问 DOM，不写 CSS |
...
后端 API 返回业务 DTO，不直接返回 `StageDocumentWorkbenchViewModel`。前端 Adapter 在页面编排层完成映射。

`P3 Design Lab` 的首版映射如下：

| 后端 API / DTO | 前端 Adapter 输入 | ViewModel 输出片段 |
| --- | --- | --- |
| `GET /input-packages` 返回的 `P3DesignLabInputPackage` | `inputPackage` | `inputFacts`、`identity.upstream` |
| `POST /sessions` 返回的 `P3DesignLabSession` | `session` | `identity`、`header`、`actions` |
| `POST /sessions/{id}/generate` 返回的 `design_document` | `session.design_document` | `product.document` / `StandardDocumentViewModel` |
| `POST /sessions/{id}/generate` 返回的 `design_baseline` | `session.design_baseline` | `product.outline`、`projection.sourceStateId` |
| `POST /sessions/{id}/check` 返回的 `check_result` | `session.check_result` | `quality` / `StageQualityGateViewModel` |
| `workorder_projection` | `session.workorder_projection` | `projection` / `StageOutputProjectionViewModel` |
| `POST /sessions/{id}/turns` 返回的轮次结果 | `SoftwareDesignTurn` | `interaction.lastTurn`、`product` 增量、`quality` 增量 |
...
```text
设计基线 ready_to_freeze / frozen
  -> 投影模块读取模块、接口、数据、状态、约束和追溯
    -> 生成 ModuleWorkorderBatchPackage
      -> 人工审阅工单包
        -> 人工触发 push_to_p4
          -> P4 接收供给目标 / 工具需求
```
...
- `components/stageWorkbench/` 只放跨阶段共性组件和 ViewModel 类型，不引用 `P3` 专用 DTO。
...
2. 页面只显示 `P2` 新版冻结包，不显示旧规格池入口。
3. 用户能选择冻结需求规格说明并创建设计会话。
4. 左侧能查看需求规格说明正文。
5. 用户能通过自然语言配置或短指令影响设计生成。
6. 右侧软件设计说明像正式 A4 文档，可以审阅。
7. 系统能生成 `SoftwareDesignBaseline v2`。
8. 系统能从同一基线投影 `P4` 模块工单预览。
9. 设计检查能解释缺口。
10. 旧 `/xx-p3` 不被破坏。
