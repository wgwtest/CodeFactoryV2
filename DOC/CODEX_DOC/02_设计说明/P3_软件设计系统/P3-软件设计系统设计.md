...
| `DesignMorphCanvasPlatform` | Canvas 视口平台，承载拖拽、缩放、命中、文本布局和渲染调度 |
| `MorphWindowTrack` | 顶部形态滑窗轴，切换相邻形态窗口 |
...
`canvasViewportState` 是前端展示状态，不属于后端权威设计事实。它可以保存用户视口偏好、最近选中对象和工作区恢复信息，但不得影响 `SoftwareDesignPackage` 的业务内容。
