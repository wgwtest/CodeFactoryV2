...
- `Registry Domain` 中登记的“工具”，默认是可复用最小资产，而不是完整页面
- 因此 `ToolDefinition` 必须能表达：
  - 这是前端元组件、后端模块、服务接口还是模板包
  - 它是被 `import`、`include_router`、`call_http_api` 还是 `mount_page`
  - 它依赖宿主提供运行时，还是自带依赖一起交付
...
这里的目标不是让 `P5` 猜这个工具如何使用，而是让 `P4` 明确告诉 `P5`：
