# P4 工具仓库设计-260419-1040-数据与投影模型补充

> 归档说明：本文件归档自 `docs/superpowers/specs/2026-04-18-p4-data-and-projection-model-design.md`，作为 `P4-工具仓库设计.md` 的数据与投影模型补充文档。后续若工作层文档继续迭代，以本文件为正式补充基线；稳定结论应回写主设计文档。
...
- `tool_granularity` 用于区分：
  - `atomic`
  - `composite`
  - `page_level`
- `P4` 的默认工具口径应优先指向 `atomic`
- 第一批真实落地验证中，`page_level` 资产不应替代前端元组件或后端模块成为默认样例

推荐进一步固定以下字段语义：

- `tool_form_id`
  - 表达工具的业务技术形态
  - 例如 `frontend_component / backend_module / service_api / template_bundle`
- `packaging_type`
  - 表达交付包装方式
  - 例如 `source_package / build_artifact / http_endpoint / descriptor_only`
- `integration_mode`
  - 表达宿主如何接入
  - 例如 `import_component / import_module / include_router / call_http_api / mount_page`
- `dependency_policy`
  - 表达运行依赖由谁提供
  - 例如 `peer / bundled / external`
- `runtime_dependencies`
  - 记录实际依赖项与版本约束
  - 例如 `react@18 / antd@5`
- `host_constraints`
  - 记录宿主框架、样式系统、运行环境等约束
- `exposed_contract_ref`
  - 指向 `props schema / api schema / event schema`
- `artifact_manifest_ref`
  - 指向真实产物清单与版本信息

这组字段的目的，是让 `P4` 能回答“这个工具究竟如何被别的系统使用”，而不是只回答“它叫什么名字”。
