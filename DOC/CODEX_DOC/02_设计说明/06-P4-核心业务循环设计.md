...
> 归档说明：本文件归档自 `docs/superpowers/specs/2026-04-18-p4-core-business-cycle-design.md`，作为 `P4` 后端演进设计的正式引用入口之一。后续若工作层文档继续迭代，以本文件为正式归档基线。自 `2026-04-19` 起，本文件作为 `P4` 总体设计下的专题设计之一按逻辑顺序归位。
...
`P5` 装配失败、资产不匹配、构建阻塞等反馈，默认进入 `P3` 的仲裁队列，而不是直接进入 `P4` 的命令队列。

标准路径应固定为：

`P5 问题报告 -> P3 仲裁 -> 设计重开 / P5 规则整改 / 新供给任务下发到 P4`

只有当 `P3` 明确判定为供给缺口时，`P4` 才收到新的外部命令。这个命令的语义不是“把旧工具改成适合这次装配”，而是“针对新的业务目标提供新的供给结果”。

这里同样要保留一个批判性边界：

- `P4` 可以继续运行 `P4.3` 自演进和内部质量修订
- 但这些自循环不是 `P5` 对 `P4` 的直接修订回流
- 任何面向业务语义的新目标，必须经 `P3` 重新解释和签发
