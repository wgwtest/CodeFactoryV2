...
- 当前状态：待用户确认
- 目标路由：`/p3-design-lab`
- 页面归属：`P3` 软件设计系统，`P3 v2` 虚规到软设核心能力原理验证台
...
| 事实源 | 对本版原型的约束 |
| --- | --- |
| 用户关于 P3 偏离主干的批注 | 当前 P3 过于偏审批流，应回到“虚规到软设”的核心能力 |
| 用户关于 P3 三块关系的说明 | 左上展示虚规，左下放自然语言配置 / CLI，右侧突出软设正文 |
| `P3-Design-Lab-v2设计.md` | 明确独立路由、输入契约、对象模型、服务边界和验收口径 |
| `P2-可配置需求规格说明编写系统设计.md` | 学习 P2 的“标准文档正面产物 + 后台语义状态”方法 |
...
| 左上虚规 | 左侧高度约 `70%` | 展示 P2 冻结需求规格说明，支持条款定位 |
...
3. 左下 CLI 用于配置和校正转换策略，权重低于虚规和软设。
4. 顶部明确写出“只消费 P2 新版冻结包，不兼容旧规格池”。

**需要用户判断：**

1. 左侧虚规和右侧软设的权重是否符合直觉。
...
![02 虚规到软设条款映射态](./02-1920x1080-虚规到软设条款映射态.png)
...
2. 设计基线完成态是否需要保留左侧虚规和 CLI，还是可以在完成态进一步扩大右侧输出区。
...
| 左下 CLI | `P3DesignTurn` 输入和响应 | 控制转换策略，不替代虚规输入 |
...
for item in \
  ...
  "trace|02-1920x1080-虚规到软设条款映射态.png" \
...
3. 总览态能表达左上虚规、左下 CLI、右侧软设正文的权重关系。
4. 条款映射态能表达虚规条款到软设章节、模块和 `Design Patch` 的关系。
