...
> 工作层说明：本文件是 `P5.1` 的专项设计草案，已在同一轮同步到正式文档 `DOC/CODEX_DOC/02_设计说明/P5_软件构建系统/P5.1-最小构建闭环设计.md`。若后续继续修改本文件，必须同步回写正式文档根目录。
...
- `/xx-p3-doc-sim` 在生成 `P3 DOC` 模拟输出后，必须立即创建一张新的 `P5` 草稿交付主单，再返回 `/build`
- `/xx-p4-supply-sim` 在生成 `P4 Supply` 模拟输出后，必须立即返回 `/build`，让当前 `P5` 工作台重新装载供给候选
- 首版样例统一采用“基于地理信息系统的通视分析软件”，禁止继续使用“P5 设计模拟输出”之类对用户无语义价值的占位命名
...
`P5.1` 原型验证阶段允许提供“一键清空当前 `P5` 交付”的测试辅助动作，但其作用域必须被严格限制。

清空动作只允许回收当前 `P5` 运行态数据：
...
执行清空后，页面必须同步把当前主单作用域层整体复位为空状态，不允许继续持有已经失效的 `current_delivery_order_id`、当前尝试或批阅上下文。
...
`P5.1` 不单独定义整套 `P5` 视觉风格。

本节点只负责记录两件事：

1. 继承 `P5` 阶段级工作台风格
2. 说明本节点相对 `P5` 阶段风格的局部例外
...
- 左侧主单队列 + 右侧工作区的双栏结构
- 订单激活卡采用实色浅表面高亮，不采用重渐变横幅
- 输入绑定区和批阅区应保持高可读、低噪声，不做风格炫技
- 运行日志区允许保留局部深色日志容器，用于表达执行器 / 终端语义
- 页头采用向 `P4` 看齐的浅色摘要头，操作按钮集中在右侧摘要区，不以粗颗粒统计横条作为主要识别手段

对 `P5.1` 的原型样式约束固定为：

- 不以大面积深色英雄区作为本节点正式视觉基线
- 不以径向渐变背景和深色头图区作为主要识别手段
- 如果后续新增页内工作区切换，应直接继承 `P4` 的分段式实色导航风格
...
- 切换当前主单
- 查看该主单最近一次批阅结果
- 一键清空当前 `P5` 交付运行态，但不删除 `P3 DOC` / `P4 Supply` 输入候选
...
- 显式确认本次输入组合
- 当 `/xx-p3-doc-sim` 新建主单返回后，默认进入该新主单作用域
- 当 `/xx-p4-supply-sim` 返回后，必须看到最新供给候选已进入当前工作台可绑定集合
...
- `/xx-p3-doc-sim` 不得只停留在“登记设计输入源”；还必须继续调用 `POST /api/software-build/orders` 创建草稿主单
- `/xx-p4-supply-sim` 不得停留在独立模拟页；生成完成后必须返回 `/build`，由工作台重新拉取输入候选
...
阶段级工作台风格放在 `DOC/CODEX_DOC/02_设计说明/P5_软件构建系统/P5-软件构建系统设计.md` 中固定。
