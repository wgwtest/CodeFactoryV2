...
- `/xx-p3-doc-sim` 的主按钮只负责生成 `P3 DOC` 模拟输出，不得隐式创建 `P5` 主单，也不得自动跳转到 `/build`
- `/xx-p4-supply-sim` 的主按钮只负责生成 `P4 Supply` 模拟输出，不得自动跳转到 `/build`
- 从模拟页进入 `P5` 必须由用户显式点击导航入口决定，不能把“生成数据”和“切换页面”绑成同一个按钮动作
...
- 显式确认本次输入组合
- 模拟页生成完成后，用户返回 `P5` 时必须能看到最新输入候选已进入可绑定集合
...
- `/xx-p3-doc-sim` 与 `/xx-p4-supply-sim` 的主按钮都只能完成各自输入源登记，不能混入自动跳页或隐式主单创建
- `P5` 工作台消费这些输入时，必须由工作台自身显式执行后续动作，而不是由模拟页替代
