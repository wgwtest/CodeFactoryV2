...
- `document_projection` 按事实语义投影到当前 81433 模板锚点，覆盖 `1 总则`、`2 项目概述`、`3 功能需求`、`4 数据需求`、`5 非功能需求`、`6 验收准则`。
- `document_projection` 对部署分析、边界排除、混合非功能枚举做优先级处理：部署分析归核心功能项，边界/不做范围归软件定位，内网部署/权限审计/追溯分别归部署、安全、质量约束。
