        ...
        *,
        ...
        provider_response = {
            "organizer_interpretation": {
                ...
                "intent": "brainstorm_requirement_decision_state",
                "confidence": "medium",
            },
            ...
            "template_shape_assessment": {
                "shape_type": "coarse_grained_extensible",
                "reason": "Brainstorm v1 先维护决策状态，再将稳定内容投影到章节正文。",
            ...
            "target_anchor_plan": [
                {
                    "plan_id": "BRAINSTORM-AP-001",
                    "decision_type": "append_existing_clause",
                    "template_clause_id": str(active_node.get("node_id") or "SPEC-REQ-1.1").removeprefix("SPEC-"),
                    "canonical_clause_heading": active_section,
                    "display_heading": active_section,
                    "reason": "Brainstorm v1 使用决策状态章节投影作为正文锚点。",
                    "confidence": "medium",
                    "anchor_path": str(active_node.get("node_id") or "SPEC-REQ-1.1").removeprefix("SPEC-"),
                }
            ],
