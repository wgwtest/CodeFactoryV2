...
def _module_name(adapter_module: str) -> str:
    ...
    if adapter_module.startswith("adapters."):
        return f"app.orchestrators.{adapter_module}"
    return adapter_module


def _load_module(manifest: OrchestratorPluginManifest):
    ...
    if spec is None or spec.loader is None:
        raise ModuleNotFoundError(f"cannot load orchestrator plugin adapter module: {module_path}")
...
def _plugin_module_path(manifest: OrchestratorPluginManifest, adapter_module: str):
    plugin_dir = _plugin_dir(manifest)
    return plugin_dir / f"{adapter_module.replace('.', '/')}.py"


def _plugin_dir(manifest: OrchestratorPluginManifest):
...
def _plugin_package_name(manifest: OrchestratorPluginManifest) -> str:
    safe_id = "".join(character if character.isalnum() else "_" for character in manifest.plugin_id)
    return f"_codefactory_plugin_{safe_id}"
