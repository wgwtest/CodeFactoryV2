...
test("renders unified knowledge browser, filters by type, and reuses the common detail drawer", async () => {
  ...
  expect(await screen.findByText("节点：4")).toBeInTheDocument();
  expect(await screen.findByText("关系：2")).toBeInTheDocument();
  ...
  expect(await screen.findByText("相关阶段/约束")).toBeInTheDocument();
  ...
  await waitFor(() =>
    expect(getMock).toHaveBeenCalledWith("/knowledge/archive/20161116-nas/items/process-service-interoperability/graph"),
  );
});
